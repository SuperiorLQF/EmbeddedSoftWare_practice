...
#include<wait.h>
#include<string.h>
#define FIFO0W "/home/lqf/lab3/object/fifo0r"
#define FIFO0R "/home/lqf/lab3/object/fifo0w"
int main(){
    //等待管道创建完毕
    int fdW,fdR,i;
    int nonnamepip[2];
    ...
    if(pid!=0){//主进程,用来显示接受的消息
        while(1){
            ...
            printf("[in proc %d] [Client1]:receive chars from nonnamepipe\n%s\n",getpid(),buffer);
        }
    }
        ...
        if(i==0){//读管道进程
            while(1){
                ...
                read(fdR,buffer,80);
                close(fdR); 
                printf("[in proc %d] [Client1]:send chars to nonnamepipe\n",getpid());
            ...
            while(1){
                ...
                printf("[in proc %d] [Client1]:send chars to [Server]\n",getpid());
